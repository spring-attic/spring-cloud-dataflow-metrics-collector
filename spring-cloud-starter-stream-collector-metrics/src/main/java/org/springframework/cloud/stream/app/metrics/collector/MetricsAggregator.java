...
/**
 * Listens for {@link ApplicationMetrics} coming from the metrics channel. Transform the payload into an {@link Stream} hierarchical model to be consumed by the server.
 * This class handles conversion, update of deltas for the sendCount, as well as invalidation of stale entries
...
public class MetricsAggregator {
	...
	public MetricsAggregator(@Qualifier("streamCache") Cache<String,Stream> streamCache, @Qualifier("rawCache") Cache<String,ApplicationMetrics> rawCache){
	...
	private Stream convert(ApplicationMetrics metrics){
		Assert.notNull(metrics.getProperties().get(ApplicationMetrics.STREAM_NAME), "Missing STREAM_NAME from metrics properties");
		Assert.notNull(metrics.getProperties().get(ApplicationMetrics.APPLICATION_NAME), "Missing APPLICATION_NAME from metrics properties");
		Assert.notNull(metrics.getProperties().get(ApplicationMetrics.APPLICATION_GUID), "Missing APPLICATION_GUID from metrics properties");

		Stream stream = new Stream((String)metrics.getProperties().get(ApplicationMetrics.STREAM_NAME));
		Application application = new Application((String)metrics.getProperties().get(ApplicationMetrics.APPLICATION_NAME));

		Double incomeRate = YANUtils.cast(metrics.getProperties().get(INPUT_METRIC_NAME),Double.class).orElse(0.0);
		Double outgoingRate =  YANUtils.cast(metrics.getProperties().get(OUTPUT_METRIC_NAME),Double.class).orElse(0.0);
		Integer instanceIndex = YANUtils.cast(metrics.getProperties().get(ApplicationMetrics.INSTANCE_INDEX),Integer.class).orElse(0);

		Instance instance = new Instance(metrics.getProperties().get(ApplicationMetrics.APPLICATION_GUID).toString(),incomeRate,outgoingRate);
		instance.setProperties(metrics.getProperties());
